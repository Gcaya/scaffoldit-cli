...
// This class is based off of what can be found here: https://github.com/dotnet/aspnetcore/blob/main/src/Shared/Process/ProcessEx.cs
internal sealed class ProcessEx : IDisposable
{
    ...
    public ProcessEx(Process proc, TimeSpan timeout, ITestOutputHelper output = null)
    {
        _output = output;
    ...
    public static ProcessEx Run(string workingDirectory, string command, string args, IDictionary<string, string>? envVars = null, TimeSpan? timeout = default, ITestOutputHelper output = null)
    {
        ...
        return new ProcessEx(proc, timeout ?? DefaultProcessTimeout, output);
    }
    ...
    public void WaitForExit(bool assertSuccess, TimeSpan? timeSpan = null)
    {
        ...
        if (!exited)
        {
            lock (_testOutputLock)
            {
                Console.WriteLine($"The process didn't exit within the allotted time ({timeSpan.Value.TotalSeconds} seconds).");
            }
